...
AUDIO_GEN_SLOT_TOKEN_ID = 151656
AUDIO_DELAY_SLOT_TOKEN_ID = 151662
...
class MossTTSModel:
    ...
    def ensure_seq_len(self, n: int) -> int:
        """Grow the KV cache + rope tables so `n` positions fit;"""
        n = int(n)
    ...
    def text_logits_2way(self, h) -> torch.Tensor:
        """Only the two audio-phase text logits:"""
        h = self._as_last_hidden(h)
        if h.dim() == 3:
            if h.shape[0] != 1:
                raise ValueError("batch must be 1")
    ...
    def audio_logits(self, h) -> torch.Tensor:
        ...
        if per_head[0].dim() == 1:
            out = torch.stack(per_head, dim=0)
        else:
            out = torch.stack(per_head, dim=1)
