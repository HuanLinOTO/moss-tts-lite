...
def replicate_record(model, input_ids, max_new_tokens, seed=SEED,
    ...
    for time_step in range(max_new_tokens):
        ...
        lt_raw = model.text_logits(h).float().clone()
        la_raw = model.audio_logits(h).float().clone()
        if time_step < record_steps:
            rec_lt.append(lt_raw.cpu())
            rec_la.append(la_raw.cpu())

        lt = (lt_raw / TEXT_TEMPERATURE).unsqueeze(0)
        ...
        if sampling_audio_mask.sum() > 0:
            ch0 = la[0:1][sampling_audio_mask[:, 0]]
            rest = la[1:][sampling_audio_mask[0, 1:]]
        ...
        if time_step < record_steps:
            rec_rows.append(current_input_ids[0, 0].cpu())
    ...
    rec = {
        "logits_text": torch.stack(rec_lt) if rec_lt else None,
        "logits_audio": torch.stack(rec_la) if rec_la else None,
        "rows": torch.stack(rec_rows) if rec_rows else None,
    }
...
def phase_a(model, zh_ids):
    ...
    with torch.inference_mode():
        ...
        for t in range(1, gt_rows.shape[0]):
            hs = model.step(gt_rows[t - 1].unsqueeze(0))
            mine_text.append(model.text_logits(hs.last_hidden).float())
            mine_audio.append(model.audio_logits(hs.last_hidden).float())
